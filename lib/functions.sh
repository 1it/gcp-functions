...
function gcp_compute_internal_address_create() {
    ...
    if [[ $(gcloud compute addresses list --format="value(NAME)" --filter="NAME:$NAME") != "$NAME" ]]; then
        gcloud compute addresses create "$NAME" --subnet=default --region "$GCP_REGION"
    else
...
function gcp_compute_address_get_ip() {
    NAME=$1; REGION=$2; 
    if [ -z "$REGION" ]; then REGION="${GCP_REGION}"; fi
    gcloud compute addresses describe "$NAME" --region "${GCP_REGION}" --format="value(ADDRESS)" 2>/dev/null
}

function gcp_compute_global_address_get_ip() {
    NAME=$1;
    gcloud compute addresses describe "$NAME" --global --format="value(ADDRESS)" 2>/dev/null
}
...
function gsutil_mb() {
    NAME=$1; LOCATION=$2;
    if ! $(gsutil ls gs://${BUCKET_NAME} 2>&1 | grep -wq "$NAME"); then
        gsutil mb -l ${LOCATION} gs://${NAME}
    else
        echo "Nothing to do. Bucket $BUCKET_NAME already exists"
    fi
}

function cloud_sql_pg_instance_suffix() {
    # Postgres instance name must be unique, GCP doesn't allow to reuse the same name even after instance deletion.
    if [ -e "cloudsql.suffix" ]; then
        PG_INSTANCE_SUFFIX=$(<"cloudsql.suffix")
    else 
        export PG_INSTANCE_SUFFIX="0$(( $RANDOM % 10000 ))"; echo "${PG_INSTANCE_SUFFIX}" > "cloudsql.suffix"
    fi
}
...
function kubectl_create_secret_from_file() {
    NAME=$1; FILE=$2;
    if [[ $(kubectl --namespace "${K8S_NS:-default}" get secrets "$NAME" -o jsonpath="{.metadata.name}") != "$NAME" ]]; then
        kubectl --namespace "${K8S_NS:-default}" create secret generic "$NAME" \
                --from-file="$FILE"="$FILE"    
    else
...
function kubectl_create_secret_docker_registry() {
    ...
    if [[ $(kubectl --namespace "${K8S_NS:-default}" get secrets "$NAME" -o jsonpath="{.metadata.name}") != "$NAME" ]]; then
        kubectl --namespace "${K8S_NS:-default}" create secret docker-registry "$NAME" \
...
function kubectl_create_config_map() {
    ...
    if [[ $(kubectl --namespace "${K8S_NS:-default}" get configmap "$NAME" -o jsonpath="{.metadata.name}") != "$NAME" ]]; then
        kubectl --namespace "${K8S_NS:-default}" create configmap "$NAME" \
...
function kubectl_create_deployment() {
    FILE="$1"
    if [[ $(kubectl --namespace "${K8S_NS:-default}" get deployment "$NAME" -o jsonpath="{.metadata.name}") != "$NAME" ]]; then
        kubectl --namespace "${K8S_NS:-default}" create -f "$FILE"
    else
...
function kubectl_deploy() {
    FILE="$1"
    kubectl --namespace "${K8S_NS:-default}" apply -f "$FILE"
    kubectl --namespace "${K8S_NS:-default}" rollout status --request-timeout="5m" -f "$FILE"
}
